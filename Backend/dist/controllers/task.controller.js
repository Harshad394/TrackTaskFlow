...
import { createTaskSchema, moveTaskSchema, taskFilterSchema, updateTaskSchema, } from "../validators/task.validator.js";
const isProjectMember = (project, userId) => {
    return Boolean(userId && project?.members.some((member) => member.userId.toString() === userId));
};
...
const createTaskNotification = async ({ recipient, actor, project, task, type, title, message, }) => {
...
export const createSectionTask = async (req, res) => {
    try {
...
export const listSectionTasks = async (req, res) => {
    try {
...
export const updateTask = async (req, res) => {
    try {
        ...
        const task = await Task.findByIdAndUpdate(req.params.taskId, { $set: validatedData }, { new: true, runValidators: true });
        if (!task) {
            return res.status(404).json({ message: "Task not found" });
        }
        await Activity.create({
...
export const moveTask = async (req, res) => {
    try {
        ...
        const targetTasks = sourceSectionId === targetSectionId
            ? [...sourceTasks]
        ...
        targetTasks.splice(insertAt, 0, req.task);
        const sourceUpdates = sourceSectionId === targetSectionId
            ? []
...
export const listTaskActivities = async (req, res) => {
    try {
...
export const getProjectTaskStats = async (req, res) => {
    try {
